...
export default function LowStockTable({ threshold = 5 }: Props) {
  ...
  return (
    <section className="rounded-none border border-krov-smoke bg-krov-graphite overflow-hidden">
      <header className="px-5 py-4 border-b border-krov-smoke/70 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-base font-semibold text-krov-bone">Bajo stock</h2>
...
function ViewToggle({
  ...
  return (
    <div
      ...
      className="inline-flex self-start rounded-none border border-krov-smoke bg-krov-graphite p-0.5"
    >
      {options.map((opt) => {
          ...
          <button
            ...
            className={
              ...
              (active
                ...
                : "text-krov-ash hover:text-krov-rose")
            }
          >
            {opt.label}
          </button>
            ...
            return (
              <tr key={row.variant_id} className="text-krov-bone">
                ...
                <td className="px-5 py-3 text-right">
                  <span
                    className={
                      "inline-block px-2 py-0.5 rounded-none text-xs font-semibold tabular-nums " +
                      (isOut
...
function DecantsView() {
  ...
  if (pools.length === 0)
    return (
      <p className="px-5 py-10 text-center text-sm text-krov-ash">
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-krov-smoke/70">
          {pools.map((row) => {
            ...
            return (
              <tr key={row.product_id} className="text-krov-bone">
                <td className="px-5 py-3">
                  <div className="font-medium">{row.name}</div>
                  <div className="text-xs text-krov-ash">{row.brand}</div>
                </td>
                <td className="px-5 py-3 text-right">
                  <span
                    className={
                      ...
                      (empty
                        ? "bg-red-500/30 text-red-200"
                        : "bg-krov-blood/15 text-krov-rose")
                    }
                  >
                    {row.pool_ml} ml
                  </span>
                  ...
                  return (
                    <td
                      ...
                      className={
                        ...
                        (units <= 0 ? "text-red-400" : "text-krov-bone")
                      }
                    >
                      {units}
                    </td>
